...
def table_exists(table_name):
    ...
    query = (
        "SELECT name FROM sqlite_master " f"WHERE type='table' AND name='{table_name}'"
    )
    try:
...
def get_starters(players, position):
    ...
    starters = [
        ...
        if position_converter(player.element_type) == position
    ]
    return starters
...
async def get_picks(team):
    ...
    for player_data in team:
        for player in players:
            ...
            player.role = ""
            player.event_points = player.event_points * player_data["multiplier"]
...
def team_width(positions, points=False):
    ...
    for position in positions:
        if points:
            player_names = [
                PICKS_FORMAT.format(player.web_name, player.event_points, player.role)
                for player in position
            ]
        else:
            player_names = [
                MYTEAM_FORMAT.format(player.web_name, player.role)
                for player in position
            ]
...
def used_chips(chips):
    ...
    if not chips:
        return "NONE."
    used = [
        "{} (GW {})".format(chip_converter(chip["name"]), chip["event"])
        for chip in chips
    ]
    return ", ".join(used)
...
def team_printer(positions, formatter, points=False):
    ...
    for position in positions:
        ...
        for player in position:
            if points:
                player_information = (player.event_points, player.web_name, player.role)
            else:
...
async def format_myteam(user):
    ...
    goalkeeper, defenders, midfielders, forwards, bench = split_by_position(players)
...
def automatic_substitutions(user_information, players):
    ...
    substitution_ids = [
        (player["element_in"], player["element_out"])
        for player in user_information["automatic_subs"]
    ]

    substitutions = []
    for player_in_id, player_out_id in substitution_ids:
        player_in = [player for player in players if player.player_id == player_in_id][
            0
        ]
        player_out = [
            player for player in players if player.player_id == player_out_id
        ][0]
...
def picks_table(user, user_information, players):
    ...
    if point_hit < 0:
        table.add_row(
            ["Gameweek transfers", "{} ({})".format(gameweek_transfers, point_hit)]
        )
    else:
        table.add_row(["Gameweek transfers", gameweek_transfers])

    table.add_row(
        ["Points on bench", user_information["entry_history"]["points_on_bench"]]
    )
    table.add_row(
        ["Automatic substitutions", automatic_substitutions(user_information, players)]
    )
...
async def format_picks(user):
    ...
    players = sorted(
        await get_picks(user_information["picks"]), key=lambda x: x.team_position
    )

    goalkeeper, defenders, midfielders, forwards, bench = split_by_position(players)

    team_printer([goalkeeper, defenders, midfielders, forwards], PICKS_FORMAT, True)
        ...
        "\nSubstitutes: {}".format(
            ", ".join(
                [
                    "{} {}".format(
                        player.event_points,
                        click.style(player.web_name, fg=player.colour),
                    )
                    for player in bench
                ]
            )
...
@click.option(
    "--email", prompt="Email address", envvar="FPL_EMAIL", help="FPL email address"
)
@click.option(
    "--password",
    prompt=True,
...
async def importaccount(user_id, email, password):
    ...
    async def add_account(user_id, email, password):
        ...
        if account_exists(user_id):
            raise ValueError("Account with user ID {} already exists!".format(user_id))
